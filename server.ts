...
const hash = (data: Buffer | string): Buffer => createHash('sha256').update(data.toString()).digest();
...
const processServerInput = (env: Env, serverInput: ServerInput): EntityInput[] => {
  ...
  if (DEBUG) {
    ...
    env.replicas.forEach((replica, key) => {
      console.log(
        `  ${key}: mempool=${replica.mempool.length}, messages=${replica.state.messages.length}, proposal=${replica.proposal ? '✓' : '✗'}`,
      );
    });
  }
...
export { getCurrentHistoryIndex, getHistory, getSnapshot, main, processServerInput, resetHistory, runDemo };
